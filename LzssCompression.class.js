...
/* Private */
	...
	#movDictionary(/*signed int*/ val){
		if(!Number.isInteger(val)) throw "movDictionary(val) - val isn't an integer when it should be.";
	...
	#insertLiteral(/*size_t */lookIndex){
		...
		if(!Array.isArray(this.out) || !Number.isNumber(this.out_s) || this.out_s <= 0){
			this.destroyOut();
